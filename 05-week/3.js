...
function questLogger(quests, actions) {
  ...
  //action ambil dari requirement, - > tampilkan nama quest dan status
  var hasil = []
  var komplit = 'Complete'
  var on = 'on progress'
  var not = 'Failed'
  var obj = {}
  for (i = 0; i < quests.length; i++) {
    var count = quests[i].requirements.length
    ...
    if (count == 0) {
      obj.status = komplit
    } else if (count > 0 && count < quests[i].requirements.length) {
      obj.status = on
    } else if (count == quests[i].requirements.length) {
      obj.status = not
    }
    hasil.push(obj)
    obj = {}

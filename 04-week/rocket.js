...
function checkCoordinate(board) {
  ...
  tempatZ = []
  tempatY = []
  wrap = [tempatX, tempatZ, tempatY]
  for (i = 0; i < board.length; i++) {
    for (j = 0; j < board[i].length; j++) {
      if (board[i][j] == 'x') {
        tempatX.push(board[0][j] + board[i][0])
      } else if (board[i][j] == 'z') {
        tempatZ.push(board[0][j] + board[i][0])
      } else if (board[i][j] == 'y') {
        tempatY.push(board[0][j] + board[i][0])
      }
    }
  }
  return wrap
...
//[ [X][Z][Y] ]
var array = [ //0j+i0, j itu huruf i itu angka
  [" ", "A", "B", "C"],
  ["1", " ", "x", " "], //1,2
  ["2", "z", "x", "y"], //2,2
  ["3", "x", " ", " "], //3,1
